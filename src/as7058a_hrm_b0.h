...
#ifndef __AS7058A_HRM_B0_H__
#define __AS7058A_HRM_B0_H__
...
/*!
 * \file
 * \brief The AS7058A HRM B0 algorithm processes data from the AS7058 Chip Library and returns heart rate data as a
 *        result.
 */
...
/*!
 * \brief Initializes the HRM B0 library.
 *
 * The library transitions to Configuration state after initialization.
 *
 * \retval ::ERR_SUCCESS Initialized successfully.
 */
err_code_t as7058a_hrm_b0_initialize(void);

/*!
 * \brief Indicates to the HRM B0 library which sub-sample is used for acquiring PPG samples.
 *
 * When ::as7058a_hrm_b0_set_input is called, the HRM B0 library extracts the samples of the given sub-samples and uses
 * them to calculate the algorithm output.
 *
 * This function can only be called when the library is in Configuration state.
 *
 * \param[in] green_ppg_sub_sample ID of the sub-sample used to acquire green PPG samples, see ::as7058_sub_sample_ids.
 ...
 */
err_code_t as7058a_hrm_b0_set_signal_routing(as7058_sub_sample_ids_t green_ppg_sub_sample,
                                             as7058_sub_sample_ids_t ambient_sub_sample);

/*!
 * \brief Configures the HRM B0 library.
 *
 * This function can only be called when the library is in Configuration state.
 *
 ...
 * \retval ::ERR_PERMISSION Invalid state.
 */
err_code_t as7058a_hrm_b0_configure(bio_hrm_b0_configuration_t config);

/*!
 * \brief Sets which signal sample preprocessing features are enabled in the HRM B0 library.
 *
 * This function can only be called when the library is in Configuration state.
 *
 ...
 * \retval ::ERR_PERMISSION Invalid state.
 */
err_code_t as7058a_hrm_b0_enable_preprocessing(uint32_t flags);

/*!
 * \brief Configures a signal sample preprocessing feature in the HRM B0 library.
 *
 * This function can only be called when the library is in Configuration state.
 *
 ...
 * \retval ::ERR_PERMISSION Invalid state.
 */
err_code_t as7058a_hrm_b0_configure_preprocessing(as7058_appmgr_preprocessing_id_t preprocessing, const void *p_config,
                                                  uint16_t size);

/*!
 * \brief Starts a processing session in the HRM B0 library.
 *
 * The AS7058A AFE must be configured so that the sample frequency of the sub-samples selected via
 * ::as7058a_hrm_b0_set_signal_routing is either 25 Hz, 50 Hz, 100 Hz, or 200 Hz. The accelerometer must be configured
 * so that its sample frequency is either 25 Hz, 50 Hz, 100 Hz, or 200 Hz.
 *
 ...
 * \retval ::ERR_PERMISSION Invalid state.
 */
err_code_t as7058a_hrm_b0_start_processing(as7058_meas_config_t measurement_config, uint32_t acc_sample_period_us);

/*!
 * \brief Provides measurement data of the regular measurement mode to the HRM B0 library.
 *
 ...
 *                                   passed to ::as7058a_hrm_b0_start_processing. For example, if the passed
 ...
 * \param[out] p_ready_for_execution Set to ::TRUE when the HRM B0 algorithm indicated that it is ready for execution,
 *                                   ::FALSE otherwise.
 *
 ...
 * \retval ::ERR_PERMISSION      Invalid state.
 */
err_code_t as7058a_hrm_b0_set_input(const uint8_t *p_fifo_data, uint16_t fifo_data_size,
...
/*!
 * \brief Executes the HRM B0 algorithm.
 *
 * This function can only be called when the library is in Processing state.
 *
 * \retval ::ERR_SUCCESS    Execution successful. Algorithm output is available and can be read via
 *                          ::as7058a_hrm_b0_get_output.
 * \retval ::ERR_NO_DATA    Execution successful, but no updated algorithm output is available.
 * \retval ::ERR_PERMISSION Invalid state.
 */
err_code_t as7058a_hrm_b0_execute(void);

/*!
 * \brief Writes the HRM B0 algorithm output to a buffer provided by the caller.
 *
 ...
 * \retval ::ERR_PERMISSION Invalid state.
 */
err_code_t as7058a_hrm_b0_get_output(bio_hrm_b0_output_t *p_output);

/*!
 * \brief Stops the current processing session of the HRM B0 library.
 *
 ...
 * \retval ::ERR_PERMISSION Invalid state.
 */
err_code_t as7058a_hrm_b0_stop_processing(void);

/*!
 * \brief De-initializes the HRM B0 library.
 *
 * \retval ::ERR_SUCCESS De-initialization successful.
 */
err_code_t as7058a_hrm_b0_shutdown(void);

/*!
 * \brief Gets the version of the HRM B0 library.
 *
 * \return Version string.
 */
const char *as7058a_hrm_b0_get_version(void);

#ifdef __cplusplus
}
#endif // __cplusplus

#endif /* __AS7058A_HRM_B0_H__ */
